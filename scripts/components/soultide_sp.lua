...
function soultide_SP:OnUpdate(dt)
    --print("是帧还是秒：",dt) --是帧， dt = 1 /60 ，一秒60帧,onupdate是刷帧的 -- 注意不同电脑帧数不一样 我的dt = 1/33 30帧数
    if not self.inst.components.health:IsDead() then
        ...
        --判断精神值
        local sanity_mode = self.inst.components.sanity:IsInsanityMode() and 1 or -0.8    --受启蒙值反转
        sanity_mode = self.inst.components.sanity.neg_aura_absorb > 0 and math.abs(sanity_mode) or sanity_mode  --蜂王冠、珍珠之泪抑制启蒙状态的反转效果

        num = math.min((num + san * sanity_mode + hunger - sp_per * 0.9 ), 0.2 )   --* TUNING.SOULTIDE_MODE --san的百分比-心情的百分比+低饱食度的影响-自带的心情减益
