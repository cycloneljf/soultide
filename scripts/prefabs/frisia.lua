...
------------等级结算
    if exp >= scale * TUNING.SOULTIDE.FRISIA.EXPPERSCALE then
        inst.scale_exp = exp - scale * TUNING.SOULTIDE.FRISIA.EXPPERSCALE
...
-----------战斗
    ...
    local ad = 0.03
    local def = 0.02
    if inst.components.skilltreeupdater:IsActivated("frisia_skillup_iv") then  ad = 0.08 def = 0.04
    elseif inst.components.skilltreeupdater:IsActivated("frisia_skillup_iii")  then ad = 0.06 def = 0.035
    elseif inst.components.skilltreeupdater:IsActivated("frisia_skillup_ii")  then ad = 0.05 def = 0.03
    elseif inst.components.skilltreeupdater:IsActivated("frisia_skillup_i")  then  ad = 0.04 def = 0.025
    end
...
local function onhitother(inst, data)--(attacker, target, damage, stimuli, weapon, damageresolved)
    ...
    if data and data.target then
        ...
        inst.components.combat.externaldamagemultipliers:SetModifier(inst,1 +  0.005 *(1 - percent)/0.05 , "frisia_talent1") --------新的因子 不要漏掉1 + 这是乘算
        ...
        if math.random() <= c_r then
            ...
            inst.components.combat.externaldamagetakenmultipliers:SetModifier(inst,0.95,"frisia_garden_angle") --减伤，不再加算，官方的代码是pairs遍历所有key，随机的，会有概率减少成0 免疫物理伤害 寄了
        end
        ------------
        update_frisia(inst)
    end
...
--受到攻击也积累狂气, 
--积累水晶之泪--上限10层 -- 每层增加2%的伤害减免和2%的暴击伤害 --暴击怎么实现 找combat调用最终伤害
...
local common_postinit = function(inst)
	-- Minimap icon
	inst.MiniMapEntity:SetIcon( "frisia_map.tex" )
