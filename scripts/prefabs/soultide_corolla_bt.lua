...
local  function update_blackthorn (inst)
    if inst and inst.components then
        ...
        if inst.components.equippable then
            inst.components.equippable.walkspeedmult = math.min( 0.96 + 0.04 * a ,TUNING.SOULTIDE.EXTRASPEED_LIMIT) --头盔在10级左右达到最大速度
        end
        ---降低san值光环
...
inst.components.armor:InitIndestructible(0.60) --怎么设置无限？--初始54%护甲 任意充能后激活等级
