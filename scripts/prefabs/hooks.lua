...
local function _findquality(inst)
    ...
    if inst and inst.scale and inst.net_scale then
        local level =  inst.net_scale:value() or inst.scale  --注意数据类型 不要少了value()
        print("[TEST e]now level is "..level)
                -- 定义每个品质对应的颜色 RGB（0~1）
        ...
        local quality = "grey" --_default --根据等级判断品质
        if level == 1 then
            quality = "white"
        elseif level <= TUNING.SOULTIDE.T.T1 and level > 1 then
            quality = "green"
        ...
        return   quality             --"<color=" .. quality_colors[quality] .. ">" .. quality .. "</color>" 不支持
    end
    return "grey"
end
...
local function _isfull(a, t) --elimit etable
    local notfull = 0
...
local _redisplayname = function(inst, num)
    ...
    if inst.GetDisplayName then
        ...
        inst.GetDisplayName = function(self, ...)
            ...
            if tab[2] ~= -1 then
                str = str .. "\r\n" .. TUNING.SOULTIDE.LANGUAGE.GEM_E .. ": " .. string.format( "%d",tab[2]) .. "/" .. elimit --网络变量是ushortint %.1f显示不出来的
            end
            if tab[3] ~= -1 then
                str = str .. "\r\n" .. TUNING.SOULTIDE.LANGUAGE.SHADOW_E .. ": " .. string.format( "%d",tab[3]) .. "/" .. elimit
            end
            if tab[4] ~= -1 then
                str = str .. "\r\n" .. TUNING.SOULTIDE.LANGUAGE.LOST_E .. ": " .. string.format( "%d",tab[4]) .. "/" .. elimit
            end
            if tab[5] ~= -1 then
                str = str .. "\r\n" .. TUNING.SOULTIDE.LANGUAGE.MOON_E .. ": " .. string.format( "%d",tab[5]) .. "/" .. elimit
            end
            if tab[6] ~= -1 then
                str = str .. "\r\n" .. TUNING.SOULTIDE.LANGUAGE.FLESH_E .. ": " .. string.format( "%d",tab[6]) .. "/" .. elimit
            end
...
local function _check(obj, t) --物体 表
    ...
    local k = " "
    for key, value in pairs(t) do
        if string.match(obj.prefab, key) then --匹配键
    ...
    if count ~= 0 then
        print("[TEST]check the prefab is " .. obj.prefab .. " in it " )
        return true, val  --返回bool 和 有5个值的表 或者一个数字
    else
        print("[TEST]check the prefab is " .. obj.prefab  .. " not in it ")
        return false, nil
    end
end
...
local function _calculateE(inst, giver ,item, num)
    ...
    if _isfull(elimit, insttab) == false then
        ...
        if _islock(inst)  == false   and  inst.scale < TUNING.SOULTIDE.DGS_MAX_SCALE then
            inst.scale = insttab[1] + 1
            print("finish up scale with maxlv".. TUNING.SOULTIDE.DGS_MAX_SCALE)
        elseif _islock(inst) == true and  inst.scale < TUNING.SOULTIDE.DGS_MAX_SCALE and ac2 and  insttab[1] ==  nextlv - 1 then
            inst.scale = insttab[1] + 1
            print("finish up scale and quality with maxlv".. TUNING.SOULTIDE.DGS_MAX_SCALE)
        end
    end
end
...
local function _strictE(inst, item, giver,num)
    ...
    --能量满了达到要求才能放进阶 能量满了 查进阶表
    if _isfull(elimit, insttab) == true and ac2 and  insttab[1] == nextlv - 1 then
        print("[TEST e] true 3 ")
        return true
    elseif _isfull(elimit, insttab) == true and ac2 then
        print("[TEST e] false 2-please use right core")
    elseif _isfull(elimit, insttab) == true  then
        print("[TEST e] false 2-this is no core")
    end
...
local function AddEnergyTradePrefab(inst)
    ...
    if inst and inst:HasTag("soultide_E") and inst.components then ---标记可以添加的物体
        print("[INIT]success add " .. inst.prefab .."with Etrade")
